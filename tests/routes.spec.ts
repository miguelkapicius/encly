...
describe("GET /:shortCode", () => {
  it("should redirect to the original URL with status 302", async () => {
    const res = await request(app.server).get("/test123456");
  ...
  it("should increment the link’s clickCount by 1 on access", async () => {
    ...
    await request(app.server).get("/test123456");

    const link = await testDb
      .select()
      .from(linksTable)
      .where(eq(linksTable.shortCode, "test123456"))
      .limit(1);
  ...
  it("should log the click in clicksTable with IP and userAgent", async () => {
    ...
    const click = await testDb
      .select()
      .from(clicksTable)
      .where(eq(clicksTable.shortCode, "test123456"))
      .limit(1);

    console.log(click[0]);

    expect(click[0].shortCode).toBe("test123456");
  ...
  it("should return 404 when shortCode does not exist", async () => {
    const res = await request(app.server).get("/test654321");

    expect(res.status).toBe(404);
  });

  it("should return 410 when the link is expired", async () => {
    const res = await request(app.server).get("/sample4567");
